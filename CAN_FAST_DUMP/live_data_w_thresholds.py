...
# Threshold LUT generation params — loaded from service_config.h at startup.
# These are set by _load_sensor_config() below; do not edit manually.
LUT_CONFIG_STEP        = 3       # CONFIG_STEP (fallback default — overridden by service_config.h)
LUT_RF_FACTOR          = 1.47    # rf_factor   (fallback default)
LUT_BASE_STEP_M        = 0.0025  # A121 base step in metres (fixed hardware constant)
# NOTE: CONFIG_START_POINT is intentionally not used here.
# Index 0 of the A121 data array is always treated as distance 0 in our system.

# Path to the persisted config (saved next to this script)
_SCRIPT_DIR   = os.path.dirname(os.path.abspath(__file__))
_VIEWER_CFG   = os.path.join(_SCRIPT_DIR, "live_viewer_config.json")

# Candidate relative paths from this script to service_config.h
_SERVICE_CONFIG_SEARCH = [
    r"C:\\Users\\mckee\\code\\stm32G_A121_firmware\\Core\\Inc\\service_config.h",
    "../stm32G_A121_firmware/Inc/service_config.h",
    "../stm32G_A121_firmware/Core/Inc/service_config.h",
    "../../stm32G_A121_firmware/Inc/service_config.h",
    "../../stm32G_A121_firmware/Core/Inc/service_config.h",
]


def _parse_service_config(path):
    """Read CONFIG_STEP and CONFIG_RF_FACTOR from a service_config.h file.
    Handles both bare values (#define X 3) and parenthesised (#define X (3)).
    CONFIG_START_POINT is intentionally ignored — index 0 is always our distance zero.
    Returns (step, rf_factor), or raises ValueError on parse failure."""
    step = None
    rf   = None
    # Matches: #define NAME (value) or #define NAME value
    # Strips optional parens and trailing f/F suffix on floats.
    _val = r'\s*\(?\s*([\d.]+)[fF]?\s*\)?'  # captures the number
    with open(path, "r", errors="replace") as f:
        for line in f:
            ...
            if stripped.startswith("//"):
                continue
            m = re.match(r'\s*#define\s+CONFIG_STEP' + _val, line)
            if m:
                step = int(m.group(1))
            m = re.match(r'\s*#define\s+CONFIG_RF_FACTOR' + _val, line)
            if m:
                rf = float(m.group(1))
    if step is None:
        raise ValueError(f"Could not find CONFIG_STEP in {path}")
    if rf is None:
        rf = 1.47   # sensible default if not present
        print(f"⚠️  CONFIG_RF_FACTOR not found in {path}, using {rf}")
    return step, rf


def _load_sensor_config(app=None):
    """Find service_config.h, parse CONFIG_STEP, and update LUT_CONFIG_STEP global.
    Saves the resolved path to live_viewer_config.json for future runs.
    Shows a Browse dialog if the file cannot be found automatically.
    Returns (step, path_used)."""
    global LUT_CONFIG_STEP, LUT_RF_FACTOR

    saved_path = None

    # 1. Try the saved path from a previous run
    if os.path.exists(_VIEWER_CFG):
        try:
            with open(_VIEWER_CFG) as f:
                cfg = json.load(f)
            saved_path = cfg.get("service_config_path", "")
        except Exception:
            pass

    if saved_path and os.path.exists(saved_path):
        try:
            step, rf = _parse_service_config(saved_path)
            LUT_CONFIG_STEP = step
            LUT_RF_FACTOR   = rf
            print(f"✅ service_config.h loaded from saved path: {saved_path}")
            print(f"   CONFIG_STEP={step}  CONFIG_RF_FACTOR={rf}")
            return step, saved_path
        except ValueError as e:
            print(f"⚠️  Saved path parse failed: {e}")

    # 2. Try common relative paths
    for rel in _SERVICE_CONFIG_SEARCH:
        candidate = os.path.normpath(os.path.join(_SCRIPT_DIR, rel))
        if os.path.exists(candidate):
            try:
                step, rf = _parse_service_config(candidate)
                LUT_CONFIG_STEP = step
                LUT_RF_FACTOR   = rf
                _save_service_config_path(candidate)
                print(f"✅ service_config.h found automatically: {candidate}")
                print(f"   CONFIG_STEP={step}  CONFIG_RF_FACTOR={rf}")
                return step, candidate
            except ValueError as e:
                print(f"⚠️  Found {candidate} but parse failed: {e}")

    # 3. Ask the user to browse
    print("⚠️  Could not find service_config.h automatically — showing file dialog.")
    ...
    QtWidgets.QMessageBox.information(
        None,
        "Locate service_config.h",
        "Could not find service_config.h automatically.\n\n"
        "Please browse to it in the next dialog.\n"
        "The path will be saved so you only need to do this once."
    )
    ...
    if path and os.path.exists(path):
        try:
            step, rf = _parse_service_config(path)
            LUT_CONFIG_STEP = step
            LUT_RF_FACTOR   = rf
            _save_service_config_path(path)
            print(f"✅ service_config.h selected by user: {path}")
            print(f"   CONFIG_STEP={step}  CONFIG_RF_FACTOR={rf}")
            return step, path
        except ValueError as e:
            print(f"❌ Could not parse selected file: {e}")

    # 4. Fall back to hardcoded defaults
    print(f"⚠️  Using fallback default: CONFIG_STEP={LUT_CONFIG_STEP}")
    return LUT_CONFIG_STEP, None


def _save_service_config_path(path):
    """Persist the service_config.h path to live_viewer_config.json."""
    try:
        cfg = {}
        if os.path.exists(_VIEWER_CFG):
            with open(_VIEWER_CFG) as f:
                cfg = json.load(f)
        cfg["service_config_path"] = path
        with open(_VIEWER_CFG, "w") as f:
            json.dump(cfg, f, indent=2)
    except Exception as e:
        print(f"⚠️  Could not save config: {e}")

# Default candidate output directories (firmware repo layout)
_OUTPUT_INC_DEFAULT = r"C:\Users\mckee\code\stm32G_A121_firmware\Core\Inc"
_OUTPUT_SRC_DEFAULT = r"C:\Users\mckee\code\stm32G_A121_firmware\Core\Src"


def _load_output_dirs(app=None):
    """Return (inc_dir, src_dir) for threshold_lut.h and threshold_lut.c.
    Priority:
      1. Saved paths in live_viewer_config.json
      2. Default Inc/Src paths if they exist on disk
      3. Browse dialogs (saves results for next run)
    Falls back to script directory if dialogs are cancelled."""
    saved_inc = None
    saved_src = None
    ...
    if os.path.exists(_VIEWER_CFG):
        try:
            with open(_VIEWER_CFG) as f:
                cfg = json.load(f)
            saved_inc = cfg.get("output_inc_dir", "")
            saved_src = cfg.get("output_src_dir", "")
        except Exception:
            pass

    # Both saved and valid — use them
    if saved_inc and os.path.isdir(saved_inc) and saved_src and os.path.isdir(saved_src):
        print(f"✅ Output Inc dir (saved): {saved_inc}")
        print(f"✅ Output Src dir (saved): {saved_src}")
        return saved_inc, saved_src

    # Try defaults
    inc_dir = saved_inc if (saved_inc and os.path.isdir(saved_inc)) else (
        _OUTPUT_INC_DEFAULT if os.path.isdir(_OUTPUT_INC_DEFAULT) else None)
    src_dir = saved_src if (saved_src and os.path.isdir(saved_src)) else (
        _OUTPUT_SRC_DEFAULT if os.path.isdir(_OUTPUT_SRC_DEFAULT) else None)

    need_app = app is None
    if need_app:
        app = QtWidgets.QApplication.instance() or QtWidgets.QApplication(sys.argv)

    # Ask for Inc dir if needed
    if inc_dir is None:
        print("⚠️  Inc output directory not set — showing folder dialog.")
        QtWidgets.QMessageBox.information(
            None, "Select Inc Output Directory",
            "Please choose the Inc folder where threshold_lut.h should be written.\n\n"
            "Typically: …\\stm32G_A121_firmware\\Core\\Inc\n\n"
            "Your choice will be saved so you only need to do this once."
        )
        chosen = QtWidgets.QFileDialog.getExistingDirectory(
            None, "Select Inc Directory (for threshold_lut.h)",
            _OUTPUT_INC_DEFAULT if os.path.isdir(os.path.dirname(_OUTPUT_INC_DEFAULT)) else _SCRIPT_DIR,
        )
        inc_dir = chosen if (chosen and os.path.isdir(chosen)) else _SCRIPT_DIR
        print(f"{'✅' if chosen else '⚠️ '} Inc dir: {inc_dir}")

    # Ask for Src dir if needed
    if src_dir is None:
        print("⚠️  Src output directory not set — showing folder dialog.")
        QtWidgets.QMessageBox.information(
            None, "Select Src Output Directory",
            "Please choose the Src folder where threshold_lut.c should be written.\n\n"
            "Typically: …\\stm32G_A121_firmware\\Core\\Src\n\n"
            "Your choice will be saved so you only need to do this once."
        )
        chosen = QtWidgets.QFileDialog.getExistingDirectory(
            None, "Select Src Directory (for threshold_lut.c)",
            _OUTPUT_SRC_DEFAULT if os.path.isdir(os.path.dirname(_OUTPUT_SRC_DEFAULT)) else _SCRIPT_DIR,
        )
        src_dir = chosen if (chosen and os.path.isdir(chosen)) else _SCRIPT_DIR
        print(f"{'✅' if chosen else '⚠️ '} Src dir: {src_dir}")

    _save_output_dirs(inc_dir, src_dir)
    return inc_dir, src_dir


def _save_output_dirs(inc_dir, src_dir):
    """Persist the Inc/Src output directories to live_viewer_config.json."""
    try:
        cfg = {}
        if os.path.exists(_VIEWER_CFG):
            with open(_VIEWER_CFG) as f:
                cfg = json.load(f)
        cfg["output_inc_dir"] = inc_dir
        cfg["output_src_dir"] = src_dir
        with open(_VIEWER_CFG, "w") as f:
            json.dump(cfg, f, indent=2)
    except Exception as e:
        print(f"⚠️  Could not save output dirs: {e}")


# Output file names (filenames only — directories resolved at runtime)
...
"""
    ...
    inc_dir, src_dir = _load_output_dirs()
    out_h = os.path.join(inc_dir, OUTPUT_H)
    out_c = os.path.join(src_dir, OUTPUT_C)
...
class LiveDataViewer(QtWidgets.QMainWindow):
    def __init__(self, port="COM5", baud=2000000, debug=False,
        ...
        # Load sensor config from service_config.h (sets LUT_CONFIG_STEP)
        _, self._sensor_config_path = _load_sensor_config()

        # range_values: index 0 = 0 mm, step size driven by CONFIG_STEP
        # Formula matches firmware: dist_m[i] = i * CONFIG_STEP * 0.0025 / rf_factor
        self.range_values = (
            np.arange(NUM_POINTS, dtype=np.float32)
            * LUT_CONFIG_STEP * LUT_BASE_STEP_M / LUT_RF_FACTOR
        )
        ...
        # Update config bar labels with whatever was loaded
        self._update_cfg_labels(self._sensor_config_path)
    ...
    def _init_controls(self, layout):
        ...
        cfg_label = QtWidgets.QLabel("LUT Config:")
        cfg_label.setStyleSheet("font-weight:bold;font-size:11px;color:#ffffff;")
        cfg_row.addWidget(cfg_label)

        self.cfg_step_label  = QtWidgets.QLabel(f"STEP={LUT_CONFIG_STEP}")
        self.cfg_rf_label    = QtWidgets.QLabel(f"RF={LUT_RF_FACTOR}")
        self.cfg_src_label   = QtWidgets.QLabel("")
        for lbl in (self.cfg_step_label, self.cfg_rf_label, self.cfg_src_label):
            lbl.setStyleSheet("font-size:11px;color:#ffffff;padding:0 6px;font-weight:bold;")
            cfg_row.addWidget(lbl)

        edit_cfg_btn = QtWidgets.QPushButton("⚙️ Edit Sensor Config")
        edit_cfg_btn.setStyleSheet("font-size:11px;padding:3px 8px;background:#e8e8e8;")
        edit_cfg_btn.clicked.connect(self._open_sensor_config_dialog)
        cfg_row.addWidget(edit_cfg_btn)

        browse_cfg_btn = QtWidgets.QPushButton("📂 Browse service_config.h")
        browse_cfg_btn.setStyleSheet("font-size:11px;padding:3px 8px;background:#e8e8e8;")
        browse_cfg_btn.clicked.connect(self._browse_service_config)
        cfg_row.addWidget(browse_cfg_btn)
        ...
        self.bp_info_label.setStyleSheet("color:#ffffff;font-size:12px;padding:2px 4px;")
        layout.addWidget(self.bp_info_label)

    # ── Sensor config helpers ─────────────────────────────────────────────────
    def _update_cfg_labels(self, path=None):
        self.cfg_step_label.setText(f"STEP={LUT_CONFIG_STEP}")
        self.cfg_rf_label.setText(f"RF={LUT_RF_FACTOR}")
        if path:
            short = os.path.basename(os.path.dirname(path)) + "/" + os.path.basename(path)
            self.cfg_src_label.setText(f"← {short}")
        # Recompute LUT distances and x-axis with new config
        self.lut_distances_mm = _compute_lut_distances()
        self.range_values = (
            np.arange(NUM_POINTS, dtype=np.float32)
            * LUT_CONFIG_STEP * LUT_BASE_STEP_M / LUT_RF_FACTOR
    ...
    def _browse_service_config(self):
        """Let the user pick service_config.h manually and reload config."""
        global LUT_CONFIG_STEP, LUT_RF_FACTOR
        path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Locate service_config.h", _SCRIPT_DIR,
            "Header files (*.h);;All files (*.*)"
        )
        if not path:
            return
        try:
            step = _parse_service_config(path)
            LUT_CONFIG_STEP = step
            _save_service_config_path(path)
            self._update_cfg_labels(path)
            self.status_bar.showMessage(
                f"✅ Loaded service_config.h: STEP={step}"
            )
        except ValueError as e:
            QtWidgets.QMessageBox.warning(self, "Parse Error", str(e))

    def _open_sensor_config_dialog(self):
        """Manual override dialog for STEP / START / RF when not using a header file."""
        global LUT_CONFIG_STEP, LUT_RF_FACTOR

        dlg = QtWidgets.QDialog(self)
        dlg.setWindowTitle("Edit Sensor Config")
        dlg.setMinimumWidth(320)
        form = QtWidgets.QFormLayout(dlg)

        step_edit  = QtWidgets.QLineEdit(str(LUT_CONFIG_STEP))
        rf_edit    = QtWidgets.QLineEdit(str(LUT_RF_FACTOR))

        form.addRow("CONFIG_STEP:", step_edit)
        form.addRow("RF factor:", rf_edit)

        note = QtWidgets.QLabel(
            "These must match your firmware service_config.h.\n"
            "Changes apply to the LUT generated on close — not to live data."
        )
        ...
        if dlg.exec_() != QtWidgets.QDialog.Accepted:
            return
        try:
            LUT_CONFIG_STEP        = int(step_edit.text().strip())
            LUT_RF_FACTOR          = float(rf_edit.text().strip())
            self._update_cfg_labels()
            self.status_bar.showMessage(
                f"⚙️ Config updated: STEP={LUT_CONFIG_STEP}  "
                f"RF={LUT_RF_FACTOR}"
            )
        except ValueError:
            QtWidgets.QMessageBox.warning(
                self, "Invalid Input", "STEP and START must be integers; RF must be a number."
            )
    ...
    def closeEvent(self, event):
        ...
        while True:
            ...
            if choice == "write":
                result = write_lut_files(self.breakpoints)
                if result:
                    _, wh, wc = result
                    QtWidgets.QMessageBox.information(
                        self, "LUT Written",
